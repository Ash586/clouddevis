...
const CATEGORIES = [
  { id: 'getting-started', title: 'Démarrage rapide', desc: 'Premiers pas avec CloudDevis', icon: '🚀', color: '#0B3D2E', articles: 4 },
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {CATEGORIES.map((cat) => (
        <Link
          key={cat.id}
          href={`/help/${cat.id}`}
          className="group bg-white border border-[#E4E0D8] rounded-xl p-4 hover:shadow-md hover:border-[#0B3D2E]/20 transition-all"
        >
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-[13px] font-bold text-[#161616] group-hover:text-[#0B3D2E] transition">{cat.title}</div>
