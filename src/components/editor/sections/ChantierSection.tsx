  ...
  return (
    <div className="grid grid-cols-2 gap-2">
      ...
      {!hiddenFields.has('chantierSurface') && <div><label className="block text-[9px] font-bold text-[var(--sand-muted)] mb-0.5">{te('chantier.surface')}</label>
        <input type="number" min="0" className="w-full border p-2 rounded-lg text-[11px] outline-none focus:ring-2 focus:ring-[var(--green-2)]" value={doc.chantierSurface || ''} onFocus={(e) => e.target.select()} onChange={(e) => setChantierField('chantierSurface', parseFloat(e.target.value) || 0)} /></div>}
