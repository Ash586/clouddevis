...
export const EditorMetaSchema = z.object({
  ...
  artisanInfo: z.object({
    ...
    carteArtisan: z.string().default(''),
    nif: z.string().default(''),
    nis: z.string().default(''),
    ai: z.string().default(''),
  }).nullable().default(null),
  discount: DiscountSchema.default(() => ({ type: 'percentage' as const, value: 0, reason: '' })),
  stampDuty: StampDutySchema.default(() => ({ rate: 1, minAmount: 5, maxAmount: 2500 })),
  paymentDetails: PaymentDetailsSchema.default(() => ({ terms: '', iban: '' })),
  ...
  bankAgencyCode: z.string().default(''),
...
export function buildEditorMeta(doc: Record<string, unknown>): EditorMeta {
  return EditorMetaSchema.parse({
    ...
    bankAgencyCode: doc.bankAgencyCode,
