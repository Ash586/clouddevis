...
export default function AdminDashboardPage() {
  ...
  const MetricBox = ({ label, value, sub, subColor }: { label: string; value: string; sub: string; subColor?: string }) => (
    <div style={card}>
      <p style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600, marginBottom: 2 }}>{label}</p>
      <p style={{ fontSize: 24, fontWeight: 700, color: '#111827', margin: 0 }}>{value}</p>
      <p style={{ fontSize: 11, fontWeight: 500, marginTop: 2, color: subColor || '#4b5563' }}>{sub}</p>
    </div>
  );
  ...
  const subPills: Record<string, { bg: string; color: string; label: string }> = {
    ...
    FREE: { bg: '#e3e7ee', color: '#4b5563', label: 'Free' },
  };
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16, ...card }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <div style={{ width: 32, height: 32, background: '#eef1f5', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, fontSize: 10, color: '#111827' }}>CD</div>
          <span style={{ fontSize: 13, fontWeight: 700, color: '#111827' }}>Rakmana — Admin Dashboard</span>
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          <span style={{ fontSize: 10, fontWeight: 600, padding: '3px 10px', borderRadius: 20, background: '#e3e7ee', color: '#4b5563', border: '0.5px solid rgba(15,23,42,0.08)' }}>Next.js 16 + PostgreSQL</span>
      ...
      <div style={{ marginBottom: 10 }}>
        <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#94a3b8', marginBottom: 8 }}>🌍 Pays d&apos;origine</p>
        <div style={card}>
          {countryData.length > 0 ? (
              ...
              {countryData.slice(0, 6).map(c => {
                ...
                return (
                  <div key={c.country} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <span style={{ fontSize: 12, color: '#94a3b8', width: 80, textAlign: 'right', flexShrink: 0 }}>{c.country}</span>
            ...
            <p style={{ fontSize: 12, color: '#94a3b8', textAlign: 'center', padding: '12px 0' }}>Aucune donnée</p>
          )}
        </div>
      </div>
      ...
      {/* ── Recent users ── */}
      <div style={{ marginBottom: 10 }}>
        <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#94a3b8', marginBottom: 8 }}>👥 Utilisateurs récents</p>
        <div style={card}>
          {recentUsers.length > 0 ? (
            <div>
              {recentUsers.slice(0, 5).map(u => {
                ...
                return (
                  <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 0', borderBottom: '0.5px solid rgba(15,23,42,0.05)' }}>
                    <div style={{ width: 28, height: 28, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, flexShrink: 0, background: '#eef1f5', color: '#4b5563' }}>
                      {u.name.split(' ').map(n => n[0]).join('').toUpperCase().slice(0, 2)}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <p style={{ fontSize: 12, fontWeight: 700, color: '#111827', margin: 0, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{u.name}</p>
                      <p style={{ fontSize: 11, color: '#94a3b8', margin: 0 }}>{u.country} · {u.mode}</p>
                    </div>
                    ...
                    <span style={{ fontSize: 11, color: '#94a3b8', flexShrink: 0 }}>{u.createdAt}</span>
                  </div>
                );
              })}
            </div>
          ) : (
            <p style={{ fontSize: 12, color: '#94a3b8', textAlign: 'center', padding: '12px 0' }}>Aucun utilisateur récent</p>
          )}
        </div>
      </div>
      ...
      {/* ── Recent activity ── */}
      <div>
        <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#94a3b8', marginBottom: 8 }}>📋 Activité système récente</p>
        <div style={card}>
          {logs.length > 0 ? (
            <div>
              {logs.map((log, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '8px 0', borderBottom: i < logs.length - 1 ? '0.5px solid rgba(15,23,42,0.05)' : 'none' }}>
                  <div style={{ width: 8, height: 8, borderRadius: '50%', marginTop: 5, flexShrink: 0, background: actionDotColors[log.action] || '#94a3b8' }} />
                  <p style={{ fontSize: 12, color: '#4b5563', flex: 1, margin: 0 }}>{logDesc(log)}</p>
                  <span style={{ fontSize: 11, color: '#94a3b8', flexShrink: 0 }}>{timeAgo(log.createdAt)}</span>
                </div>
              ))}
            </div>
          ) : (
            <p style={{ fontSize: 12, color: '#94a3b8', textAlign: 'center', padding: '12px 0' }}>Aucune activité récente</p>
          )}
        </div>
      </div>
    </div>
