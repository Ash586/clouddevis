...
const input = { background: '#e3e7ee', border: '0.5px solid rgba(15,23,42,0.08)', borderRadius: 6, padding: '9px 12px', fontSize: 13, color: '#111827', outline: 'none', width: '100%' };
const select = { ...input, cursor: 'pointer' };
const btn = { padding: '9px 16px', borderRadius: 6, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer' };
const btnPrimary = { ...btn, background: '#eef1f5', color: '#111827', border: '0.5px solid rgba(15,23,42,0.08)' };

const statusPill: Record<string, { bg: string; color: string }> = {
  TRIAL: { bg: 'rgba(251,191,36,0.10)', color: '#d97706' },
  STANDARD: { bg: 'rgba(74,158,255,0.10)', color: '#2563eb' },
...
export default function AdminSubscriptionsPage() {
  ...
  return (
    <div>
      <h1 style={{ fontSize: 22, fontWeight: 700, color: '#111827', marginBottom: 20 }}>{t('nav.subscriptions')}</h1>
          ...
          {statuses.map(s => {
            ...
            return (
              <div key={s} style={{ ...card, textAlign: 'center', padding: '12px 10px' }}>
                ...
                <p style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600, marginTop: 2 }}>{s}</p>
              </div>
            );
          })}
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              ...
              <tbody>
                {subscriptions.map(sub => {
                  ...
                  return (
                    <tr key={sub.id} style={{ borderBottom: '0.5px solid rgba(15,23,42,0.05)' }}>
                      <td style={{ padding: '8px 12px', fontWeight: 600, color: '#111827' }}>{sub.name}</td>
                      <td style={{ padding: '8px 12px', color: '#4b5563' }}>{sub.email}</td>
                      ...
                      <td style={{ padding: '8px 12px', color: '#4b5563' }}>{sub.subscriptionEndAt || '—'}</td>
                      <td style={{ padding: '8px 12px', color: '#4b5563' }}>{sub.docCount}</td>
                    </tr>
                  );
                })}
              </tbody>
