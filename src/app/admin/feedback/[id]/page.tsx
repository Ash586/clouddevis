...
export default function AdminFeedbackDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (loading) return <div className="min-h-screen bg-[#0f1117] p-6"><div className="animate-pulse text-sm text-slate-500">Chargement...</div></div>;
  if (!fb) return (
    <div className="min-h-screen bg-[#0f1117] p-6 max-w-3xl mx-auto">
      <Link href="/admin/feedback" className="text-xs text-slate-400 hover:text-white flex items-center gap-1 mb-4"><ArrowLeft size={14} /> Retour</Link>
      <Card className="p-8 text-center bg-slate-900/50 border-slate-800"><p className="text-sm text-slate-500">Retour introuvable</p></Card>
    </div>
  );
    ...
    <div className="min-h-screen bg-[#0f1117] p-6 max-w-3xl mx-auto">
      ...
      <div className="flex items-start justify-between gap-4 mb-5">
        <div className="flex items-center gap-2">
          <Icon size={18} className={fb.type === 'BUG' ? 'text-red-400' : 'text-amber-400'} />
          <h1 className="text-lg font-black text-white">{fb.type === 'BUG' ? 'Signalement de bug' : 'Suggestion'}</h1>
        </div>
        <Badge variant={STATUS_COLORS[fb.status]}>{STATUS_LABELS[fb.status]}</Badge>
      </div>

      <Card className="p-4 bg-slate-900/50 border-slate-800 mb-4">
        <div className="grid grid-cols-2 gap-3 text-xs">
          <div>
            <p className="text-slate-500">Utilisateur</p>
            <Link href={`/admin/users/${fb.user.id}`} className="text-white font-semibold hover:underline">{fb.user.name}</Link>
          </div>
          <div>
            <p className="text-slate-500">Email du compte</p>
            <p className="text-slate-300">{fb.user.email}</p>
          </div>
          {fb.email && (
            <div>
              <p className="text-slate-500">Contact fourni</p>
              <p className="text-slate-300">{fb.email}</p>
            </div>
          )}
          {fb.user.phone && (
            <div>
              <p className="text-slate-500">Téléphone</p>
              <p className="text-slate-300">{fb.user.phone}</p>
            </div>
          )}
          <div>
            <p className="text-slate-500">Envoyé le</p>
            <p className="text-slate-300">{new Date(fb.createdAt).toLocaleString('fr-FR')}</p>
          </div>
        </div>
      </Card>
      ...
      <Card className="p-4 bg-slate-900/50 border-slate-800 mb-4">
        ...
        <p className="text-sm text-slate-200 whitespace-pre-wrap leading-relaxed">{fb.message}</p>
      </Card>
      ...
      <div className="flex flex-wrap gap-2 mb-5">
        {(['NEW', 'REVIEWED', 'RESOLVED'] as const).map(s => (
          <button type="button" key={s} disabled={saving !== null || fb.status === s}
            ...
            className={`px-3 py-1.5 text-xs font-bold rounded-lg transition disabled:opacity-40 ${fb.status === s ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400 hover:text-white'}`}>
      ...
      <Card className="p-4 bg-slate-900/50 border-slate-800">
        <p className="text-sm font-bold text-white mb-1">Répondre à l&apos;utilisateur</p>
        ...
        <textarea
          ...
          className="w-full h-28 resize-none rounded-lg border border-slate-700 bg-slate-800/50 p-3 text-sm text-slate-200 outline-none focus:ring-2 focus:ring-blue-500/40"
        />
